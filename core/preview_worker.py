...
def render_pdf(path: str, max_pages: int = 3, dpi_scale: float = 1.2) -> str:
    ...
    try:
        ...
        badge_note = f" (前 {max_pages} 頁)" if total_pages > max_pages else ""
        ...
        html_parts.append(f'<div style="width:100%; height:0; overflow:visible; position:relative; z-index:1000;"><div class="pdf-info-badge"><span>{info_line}{badge_note}</span>{open_btn}</div></div>')
